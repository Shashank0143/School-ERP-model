...
const EmployeeLeavePage = () => {
  ...
  const fetchLeaves = async () => {
    ...
    try {
      ...
      const balances = await getBalanceByUser(employeeId, "employee");
      ...
      const filteredTypes = types.filter(t => {
        ...
        if (!t.applicableTo.includes("employee")) return false;
  ...
  const handleCancelLeave = async (leaveId) => {
    ...
    try {
      await cancelEmployeeLeaveRequest(leaveId, employeeId);
  ...
  return (
    ...
    >
      ...
      <LeavePortfolioDashboard userId={employeeId} userType="employee" gender={user?.profile?.gender} refreshTrigger={refreshTrigger} />

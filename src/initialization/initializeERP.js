...
import { clearAllData, setItem, getItem, removeItem } from "../persistence/storage";
...
export const initializeERP = () => {
  ...
  try {
    ...
    // ── Fee Structures Initial Seed Fix ─────────────────────────────────────────
    const CURRENT_FEE_STRUCT_SCHEMA = 2;
    const storedFeeStructSchema = parseInt(getItem("erp_fee_struct_schema_version") || "1", 10);
    if (storedFeeStructSchema < CURRENT_FEE_STRUCT_SCHEMA) {
      console.log("[InitializationEngine] Upgrading Fee Structures schema v1 → v2");
      setItem(STORAGE_KEYS.FEE_STRUCTURES, feeStructuresSeed);
      setItem("erp_fee_struct_schema_version", CURRENT_FEE_STRUCT_SCHEMA);
    }

    // ── Missing Mock Data Seed Fix ─────────────────────────────────────────
    // Forcefully remove empty arrays created by old ensureRequiredKeys
    const CURRENT_MOCK_SCHEMA = 2;
    const storedMockSchema = parseInt(getItem("erp_mock_schema_version") || "1", 10);
    if (storedMockSchema < CURRENT_MOCK_SCHEMA) {
      console.log("[InitializationEngine] Upgrading Mock Data schema v1 → v2 (force seeding missed mock data)");
      removeItem(STORAGE_KEYS.CLUBS);
      removeItem(STORAGE_KEYS.CLUB_ENROLLMENTS);
      removeItem(STORAGE_KEYS.CLUB_ACTIVITIES);
      removeItem(STORAGE_KEYS.EVENTS);
      removeItem(STORAGE_KEYS.DOCUMENTS);
      removeItem(STORAGE_KEYS.ACHIEVEMENTS);
      removeItem(STORAGE_KEYS.TRANSPORT_ASSIGNMENTS);
      removeItem(STORAGE_KEYS.TRANSPORT_STOPS);
    ...
    seedIfEmpty(STORAGE_KEYS.FEE_STRUCTURES, feeStructuresSeed, "fee structures");
    ...
    seedIfEmpty(STORAGE_KEYS.FEE_STRUCTURES, feeStructuresSeed, "fee structures");
    seedIfEmpty(STORAGE_KEYS.ACADEMIC_CALENDAR, schoolCalendar, "academic calendar");
...
export const resetERPData = () => {
  ...
  try {
    ...
    setItem(STORAGE_KEYS.ACADEMIC_CALENDAR, schoolCalendar);
